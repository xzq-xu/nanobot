...
class AgentLoop:
    ...
    async def _run_agent_loop(
        ...
        *on_turn_saved*, when provided, is called with the full message
        list after each tool-call iteration so the caller can persist
        intermediate state.  This prevents data loss if the loop is
        interrupted mid-way (e.g. /stop, crash).
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        # consumers (CLI, channels) never see think-tag content in the stream.
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # Steering: check before launching tools — cancel all on interruption
                if injection := _drain_interruptions():
                    logger.info("Steering: interruption before tools, cancelling {} tools", len(response.tool_calls))
                    for tc in response.tool_calls:
